...
def impute_dataset(df, impute_method):
    ...
    """
    #missing_column_counts = df[df.columns[df.isnull().any()]].isnull().sum()
    #max_idxs = np.argsort(missing_column_counts.to_numpy())[::-1][:]
    #nan_cols = df.columns[max_idxs[:]].to_numpy()
    #nan_cols = df.columns[max_idxs[:]]
...
if df is not None:
    ...
    st.markdown("Top 3 categories with most missing values: " + str(missing_data_summary['top_missing_categories']))
